...
class World(object):
    ...
    def restart(self, keep=False):
        ...
        if self.vehicle_blueprint is None:
    ...
    def destroy(self, keep=False):
        ...
        for sensor in sensors:
            if sensor is not None:
                sensor.stop()
                sensor.destroy()
        if self.player is not None and not keep:
            self.player.destroy()
...
class KeyboardControl(object):
    ...
    def __init__(self, world, start_in_autopilot):
        ...
        self.constant_velocity = world.constant_velocity
    ...
    def parse_events(self, client, world, clock, sync_mode):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self._is_quit_shortcut(event.key):
                    return True
                elif event.key == K_BACKSPACE and not (pygame.key.get_mods() & KMOD_CTRL):
                    if self._autopilot_enabled:
                        ...
                        world.restart()
                elif event.key == K_BACKSPACE and (pygame.key.get_mods() & KMOD_CTRL):
                    if self._autopilot_enabled:
                        world.player.set_autopilot(False)
                        world.restart(keep=True)
                        world.player.set_autopilot(True)
                    else:
                        world.restart(keep=True)
                elif event.key == K_F1:
        ...
        if world.recording_enabled:
            ...
            with open('ctrl_files/control_test.json', 'w+', encoding='utf-8') as p:
                json.dump(self._controls_dic, p)
            np.save(f'ctrl_files/control_test_id_{world.player.id}.npy', self.controls_np)
...
def main():
    ...
    print(__doc__)
    os.makedirs('ctrl_files', exist_ok=True)
