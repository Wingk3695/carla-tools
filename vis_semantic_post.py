...
if __name__ == "__main__":
    png_path = r"H:\small_object_dataset\post_data\pinhole\ph_semantic0_0.png"
    img = cv2.imread(png_path, cv2.IMREAD_UNCHANGED)
    type_img = img[:,:,2] if img.shape[2] == 3 else img[:,:,0]  # R通道
    color_img = semantic_to_cityscapes_color(type_img)
    cv2.imshow("Semantic Cityscapes Color", color_img)
    cv2.waitKey(0)  
...
# if __name__ == "__main__":
#     # 直接读取 npz 文件
#     npz_path = r"H:\test\raw_data\cm_semantic0\cm_semantic0_0.npz"
